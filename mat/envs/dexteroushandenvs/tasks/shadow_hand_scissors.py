...
class ShadowHandScissors(BaseTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[:], self.consecutive_successes[:] = compute_hand_reward(
            ...
            self.max_episode_length, self.object_pos, self.object_rot, self.goal_pos, self.goal_rot, self.scissors_right_handle_pos, self.scissors_left_handle_pos, 
    ...
    def compute_observations(self):
        ...
        self.scissors_right_handle_pos = self.rigid_body_states[:, 26 * 2 + 1, 0:3]
        self.scissors_right_handle_rot = self.rigid_body_states[:, 26 * 2 + 1, 3:7]
        ...
        self.scissors_right_handle_pos = self.scissors_right_handle_pos + quat_apply(self.scissors_right_handle_rot, to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * 0.1)
        self.scissors_right_handle_pos = self.scissors_right_handle_pos + quat_apply(self.scissors_right_handle_rot, to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0.0)

        self.scissors_left_handle_pos = self.rigid_body_states[:, 26 * 2 + 2, 0:3]
        self.scissors_left_handle_rot = self.rigid_body_states[:, 26 * 2 + 2, 3:7]
        ...
        self.scissors_left_handle_pos = self.scissors_left_handle_pos + quat_apply(self.scissors_left_handle_rot, to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * -0.02)
...
def compute_hand_reward(
    ...
    max_episode_length: float, object_pos, object_rot, target_pos, target_rot, scissors_right_handle_pos, scissors_left_handle_pos,
    ...
    up_rew = (torch.norm(scissors_right_handle_pos - scissors_left_handle_pos, p=2, dim=-1) - 0.2) * 20
